...
export default function Smooth3DSlideshow(props: Smooth3DSlideshowProps) {
    ...
    // Hover pop — the card lifts toward the viewer (brightens, grows slightly).
    const [hovered, setHovered] = useState<number | null>(null)
    const hoverTransition = `transform 0.35s cubic-bezier(0.34, 1.3, 0.64, 1), opacity ${dur}s ${ease}`
        ...
        >
            ...
            >
                {list.map((slide, i) => {
                    ...
                    const tx = rel * (gap * 30)
                    const tz = -ax * DEPTH + (isHovered ? 90 : 0)
                    const ry = -rel * tilt
                    ...
                    const cardStyle: CSSProperties = {
                        ...
                        transform: `translate(-50%, -50%) translateX(${tx}px) translateZ(${tz}px) rotateY(${ry}deg) rotateZ(${rz}deg) scale(${sc * (isHovered ? 1.06 : 1)})`,
                        transition: isHovered ? hoverTransition : transitionCss,
                        opacity: visible ? 1 : 0,
                    ...
                    return (
                        <div
                            ...
                            onMouseEnter={
                                isStatic ? undefined : () => setHovered(i)
                            }
                            onMouseLeave={
                                isStatic ? undefined : () => setHovered(null)
                            }
                        ...
                        >
                            ...
                            <div
                                style={{
                                    ...
                                    inset: 0,
                                    background: "#000000",
                                    opacity: isActive || isHovered ? 0 : dim,
